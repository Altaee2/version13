...
# --- 🔐 دالة تشفير/فك تشفير السيشنات ---
def encrypt_session(session_str: str) -> str:
    """تشفير السيشن باستخدام base64 (يمكن تعويضه بـ Fernet لأمان أفضل)"""
    try:
    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, 'r') as f: 
                data = json.load(f)
                # 🔐 فك تشفير السيشنات عند القراءة
                for user_id, installs in data.items():
                    if isinstance(installs, list):
                        for install in installs:
                            if 'session' in install:
                                install['session'] = decrypt_session(install['session'])
                return data
...
def save_db(data):
    try:
        # 🔐 تشفير السيشنات قبل الحفظ
        encrypted_data = {}
        for user_id, installs in data.items():
            if isinstance(installs, list):
                encrypted_data[user_id] = []
                for install in installs:
                    encrypted_install = install.copy()
                    if 'session' in encrypted_install:
                        encrypted_install['session'] = encrypt_session(encrypted_install['session'])
                    encrypted_data[user_id].append(encrypted_install)
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if msg.file and msg.file.name.endswith(".json"):
            ...
            try:
                ...
                # عدد التنصيبات التي تمت إضافتها
                added_count = 0
                failed_count = 0
                users_processed = set()
                
                # التكرار على المستخدمين والتنصيبات في الملف المرفوع
                for user_id_str, user_installs in uploaded_data.items():
                    # تأكد من أن user_id هو نص
                    if not isinstance(user_id_str, str):
                        user_id_str = str(user_id_str)
                    
                    users_processed.add(user_id_str)
                    
                    # إذا المستخدم غير موجود في DB الحالية، أنشئ قائمة جديدة
                    if user_id_str not in db:
                        db[user_id_str] = []
                    
                    # التكرار على تنصيبات هذا المستخدم
                    if isinstance(user_installs, list):
                        for install in user_installs:
                            if "session" in install and install["session"]:
                                # إضافة التنصيب إلى قائمة المستخدم
                                db[user_id_str].append(install)
                                added_count += 1
                                
                                # تشغيل السيشن تلقائياً
                                try:
                                    asyncio.create_task(
                                        run_user_safely(
                                            install['session'], 
                                            install.get('api_id', API_ID), 
                                            install.get('api_hash', API_HASH), 
                                            install, 
                                            int(user_id_str), 
                                            len(db[user_id_str]) - 1
                                        )
                                    )
                                    logger.info(f"تم تشغيل سيشن المستخدم {user_id_str} تلقائياً")
                                except Exception as e:
                                    logger.error(f"فشل تشغيل سيشن المستخدم {user_id_str}: {e}")
                                    failed_count += 1
                            
                            elif "bot_token" in install: # تجاهل بيانات البوتات
                                failed_count += 1 # عدها فاشلة لأننا أزلنا الميزة
                    
                    elif isinstance(user_installs, dict): # في حال كان لديه تنصيب واحد فقط
                        if "session" in user_installs and user_installs["session"]:
                            db[user_id_str].append(user_installs)
                            added_count += 1
                            
                            try:
                                asyncio.create_task(
                                    run_user_safely(
                                        user_installs['session'], 
                                        user_installs.get('api_id', API_ID), 
                                        user_installs.get('api_hash', API_HASH), 
                                        user_installs, 
                                        int(user_id_str), 
                                        len(db[user_id_str]) - 1
                                    )
                        ...
                        elif "bot_token" in user_installs: # تجاهل بيانات البوتات
                            failed_count += 1 # عدها فاشلة لأننا أزلنا الميزة
                ...
                # إرسال تقرير
                report_msg = (
                    f"✅ **تـم رفـع وتـحـديـث قاعدة البيانات بـنـجـاح:**\n\n"
                    f"📊 **التقرير:**\n"
                    f"• التنصيبات المضافة: `{added_count}`\n"
                    f"• المستخدمين المعالجين: `{len(users_processed)}`\n"
                    f"• التنصيبات التي فشلت: `{failed_count}`\n"
                    f"• إجمالي المستخدمين الآن: `{len(db)}`"
                )
                await conv.send_message(report_msg)
                logger.info(f"تم رفع نسخة احتياطية: {added_count} تنصيبات، {failed_count} فشلت")
                
            except Exception as e:
                await conv.send_message(f"❌ **خـطأ أثناء معالجة الملف:**\n`{str(e)}`")
                logger.error(f"خطأ في رفع النسخة الاحتياطية: {e}")
                if os.path.exists(temp_file):
                    os.remove(temp_file)
        else:
            await conv.send_message("❌ **خـطأ: يـرجـى إرسـل مـلـف JSON صـحـيـح.**")
...
async def load_backup():
    """تشغيل جميع السيشنات المخزنة عند بدء البوت"""
    try:
        db = get_db()
        if db:
            total_installs = sum(len(installs) for installs in db.values())
            logger.info(f"جاري إعادة تشغيل {total_installs} تنصيب...")
            
            # ⚡ استخدام asyncio.gather لتسريع التشغيل
            tasks = []
            for uid, installs in db.items():
                for idx, info in enumerate(installs):
                    if "session" in info:
                        try:
                            task = run_user_safely(
                                info['session'], 
                            ...
                            logger.error(f"فشل إنشاء مهمة recoco للمستخدم {uid}: {e}")
            
            # تشغيل جميع المهام في آن واحد
            if tasks:
                await asyncio.gather(*tasks, return_exceptions=True)
                logger.info(f"تم إطلاق جميع المهام ({len(tasks)})")
