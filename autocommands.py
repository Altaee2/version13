...
"""
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            if data["step"] == "links":
                ...
                if not current_text.isdigit():
                    ...
                    return
                delay_input = int(current_text)
                if delay_input < 300 or delay_input > 3000:
                    await event.edit("⚠️ **الوقت يجب أن يكون بين 300 و 3000 ثانية.**")
                    return
                data["delay"] = delay_input
                data["step"] = "count"
                ...
                for link in current_links:
                    try:
                        ...
                        async def auto_post_task(t_chat, t_cnt, t_seconds, t_msg, t_link):
                            sent = 0
                            while sent < t_cnt:
                                try:
                                    await client.send_message(t_chat, t_msg)
                                    sent += 1
                                    group_publish_data["sent"][t_link] = sent
                                    group_publish_data["last_sent"][t_link] = datetime.now()
                                except:
                                    break # توقف في حال الحظر من المجموعة
                                if sent >= t_cnt: break
                                await asyncio.sleep(t_seconds)

                        task = asyncio.create_task(
                            auto_post_task(target_chat, final_count, current_delay, current_msg, link)
                        )
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            async with client.conversation(event.chat_id, timeout=60) as conv:
                ...
                # إيقاف المهام القديمة
                for task in group_publish_data["tasks"].values():
                    task.cancel()
                group_publish_data["tasks"].clear()
                group_publish_data["sent"] = {link: 0 for link in group_publish_data["links"]}

                # إعادة تشغيل النشر بالكليشة الجديدة
                for link in group_publish_data["links"]:
                    try:
                        entity = await client.get_entity(link)
                        target_chat = entity.id

                        async def auto_post_task(t_chat, t_count, t_seconds, t_msg, t_link):
                            sent = 0
                            while sent < t_count:
                                try:
                                    await client.send_message(t_chat, t_msg)
                        ...
                        task = asyncio.create_task(
                            auto_post_task(target_chat, group_publish_data["count"], group_publish_data["delay"], new_message, link)
                        )
                        group_publish_data["tasks"][link] = task
            ...
            for link in group_publish_data["links"]:
                try:
                    ...
                    msg += f"🔹 **رابط:** `{link}`\n" \
                           ...
                           f"├ المتبقي: `{remaining}`\n" \
                           f"└ الحالة: `{'نشط ✅' if link in group_publish_data['tasks'] else 'مكتمل أو متوقف 🛑'}`\n\n"
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # 10. أمر إضافة رابط جديد لقائمة النشر
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            if not group_publish_data["message"]:
                return await event.edit("⚠️ **يجب إضافة مجموعات عبر `.اضف_مجموعة` أولاً لضبط الإعدادات.**")

            await event.edit("🔗 **أرسل الرابط الجديد لإضافته للنشر التلقائي:**")
            ...
            async with client.conversation(event.chat_id, timeout=60) as conv:
                try:
                    ...
                    if new_link in group_publish_data["links"]:
                        return await event.respond("⚠️ **هذا الرابط موجود بالفعل في القائمة.**")
                    ...
                    task = asyncio.create_task(
                        auto_post_task(target_chat, group_publish_data["count"], group_publish_data["delay"], group_publish_data["message"], new_link)
                    )
                    group_publish_data["tasks"][new_link] = task

                    await event.respond(f"✅ **تمت إضافة الرابط بنجاح وبدأ النشر:**\n`{new_link}`")
                except Exception as e:
                    await event.respond(f"❌ **حدث خطأ:**\n`{str(e)}`")

        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # 11. أمر حذف رابط من القائمة
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            ...
            if not group_publish_data["links"]:
                return await event.edit("⚠️ **القائمة فارغة بالفعل.**")

            await event.edit("❌ **أرسل الرابط الذي تريد حذفه من القائمة:**")

            async with client.conversation(event.chat_id, timeout=60) as conv:
                try:
                    msg = await conv.get_response()
                    rem_link = msg.raw_text.strip()

                    if rem_link in group_publish_data["links"]:
                        if rem_link in group_publish_data["tasks"]:
                            group_publish_data["tasks"][rem_link].cancel()
                            del group_publish_data["tasks"][rem_link]
                        
                        group_publish_data["links"].remove(rem_link)
                        if rem_link in group_publish_data["sent"]: del group_publish_data["sent"][rem_link]
                        
                        await event.respond(f"✅ **تم حذف الرابط وإيقاف النشر فيه بنجاح.**")
                    else:
                        await event.respond("⚠️ **هذا الرابط غير موجود في القائمة.**")
                except:
                    await event.respond("⚠️ **حدث خطأ أو انتهى وقت الاستجابة.**")
